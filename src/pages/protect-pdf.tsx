...
const ProtectPdfPage: NextPage = () => {
  ...
  const handleProcess = async () => {
    ...
    try {
      const processed = await protectPdf(file, password);
      const blob = new Blob([processed], { type: 'application/pdf' });
      const url = URL.createObjectURL(blob);
      setDownloadUrl(url);
      setProcessedFileName(`protected_${file.name}`);
      setStatus('success');
    } catch (err) {
  ...
  const isProcessButtonDisabled = !password || !confirmPassword || password !== confirmPassword;

  return (
    <>
      <Head>
        <title>Protect PDF with Password - Free & Secure</title>
        <meta name="description" content="Add a password to your PDF file to encrypt it and protect sensitive information. Fast, client-side, and secure." />
      </Head>
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-4xl font-bold text-center mb-4">Protect PDF with a Password</h1>
        <p className="text-lg text-gray-600 text-center mb-8">
          Encrypt your PDF file by adding a password. Only those with the password will be able to open it.
        </p>
        ...
        {status === 'options' && file && (
             <div className="w-full max-w-4xl mx-auto grid md:grid-cols-2 gap-8 items-start">
                <div className="md:sticky md:top-24">
                  <h2 className="text-2xl font-bold mb-4 text-center md:text-left">File to Protect</h2>
                  <PDFPreviewer file={file} index={0} onRemove={handleStartOver} />
                </div>
                <Card>
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2"><Shield /> Set a Password</CardTitle>
                    </CardHeader>
