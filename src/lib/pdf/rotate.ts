...
import { RotateOptions, RotationAngle } from '@/components/tools/RotateOptions';
...
import { rotatePDF } from '@/lib/pdf/rotate'; // <-- New import
...
const ToolPage: NextPage<ToolPageProps> = ({ tool }) => {
  ...
  const handleFilesSelected = async (files: File[]) => {
    ...
    const needsOptions = ['split-pdf', 'compress-pdf', 'organize-pdf', 'rotate-pdf'].includes(tool.value);
    ...
    try {
      ...
      switch (tool.value) {
        ...
        case 'compress-pdf':
          ...
          break;
        case 'rotate-pdf':
          const rotatedBytes = await rotatePDF(selectedFiles[0], rotationAngle);
          resultBlob = new Blob([rotatedBytes], { type: 'application/pdf' });
          filename = `${originalName}_rotated.pdf`;
          break;
        default:
          // Mock functionality for other tools
  ...
  const renderContent = () => {
    switch (status) {
      ...
      case 'loading_preview': return ( /* ... Loading Preview UI ... */ );
      case 'arranging': return ( /* ... FileArranger for Merge PDF ... */ );
      case 'options':
        return (
            <div className="w-full grid md:grid-cols-2 gap-8 items-start">
                ...
                <div>
                    ...
                    {tool.value === 'rotate-pdf' && <RotateOptions onRotate={setRotationAngle} />}
