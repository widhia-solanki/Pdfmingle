...
export const PdfEditor = ({ file, pageIndex, objects, onObjectsChange, mode, onObjectSelect }: PdfEditorProps) => {
  ...
  const handlePointerDown = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    const newObject: DrawObject | HighlightObject = mode === 'draw' ? {
      ...
      color: { r: 0, g: 0, b: 0 }, strokeWidth: 4,
    } : {
  ...
  const handleCanvasClick = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (mode === 'text') {
      const newText: TextObject = {
        type: 'text', id: `text-${Date.now()}`, x, y, text: "New Text", size: 24, font: 'Helvetica',
        color: { r: 0, g: 0, b: 0 }, pageIndex, width: 200, height: 50, isEditing: true
      };
      onObjectsChange([...objects, newText]);
      onObjectSelect(newText);
    } else if (mode === 'shape') {
      const newShape: ShapeObject = {
        type: 'shape', id: `shape-${Date.now()}`, pageIndex, shapeType: 'rectangle', x, y, width: 150, height: 100,
        fillColor: { r: 0, g: 123, b: 255, a: 0.5 }, borderColor: { r: 0, g: 0, b: 0 }, borderWidth: 2,
      };
  ...
  const updateObject = (id: string, newProps: Partial<EditableObject>) => {
      const updatedObjects = objects.map(obj => obj.id === id ? { ...obj, ...newProps } : obj);
      onObjectsChange(updatedObjects);
  };
  
  const handleDoubleClick = (obj: TextObject) => {
    const newObjects = objects.map(o => o.id === obj.id ? { ...o, isEditing: true } : { ...o, isEditing: false });
  ...
  return (
    <div className="relative w-fit h-fit bg-white shadow-lg">
      ...
      <div
          className={cn("absolute top-0 left-0 w-full h-full z-10", (mode !== 'select') ? "pointer-events-auto" : "pointer-events-none", (mode === 'draw' || mode === 'highlight' || mode === 'shape') && 'cursor-crosshair', mode === 'text' && 'cursor-text')}
      ...
      >
        ...
        {!isLoading && objects.filter((obj): obj is TextObject | ImageObject | ShapeObject => (obj.type === 'text' || obj.type === 'image' || obj.type === 'shape') && obj.pageIndex === pageIndex).map(obj => (
            <Rnd
                ...
                onResizeStop={(e, direction, ref, delta, position) => { updateObject(obj.id, { width: parseInt(ref.style.width), height: parseInt(ref.style.height), ...position }); }}
            ...
            >
                {obj.type === 'text' && (
                  obj.isEditing ? (
                    <textarea value={obj.text} onChange={(e) => handleTextChange(obj.id, e.target.value)} onBlur={() => handleTextBlur(obj)} autoFocus style={{ width: '100%', height: '100%', border: 'none', outline: 'none', padding: 0, margin: 0, resize: 'none', background: 'transparent', fontSize: `${obj.size}px`, color: `rgb(${obj.color.r}, ${obj.color.g}, ${obj.color.b})`, fontFamily: obj.font, }} />
                ...
                {obj.type === 'image' && (<img src={URL.createObjectURL(new Blob([obj.imageBytes]))} alt="user upload" className="w-full h-full object-contain" />)}
                {obj.type === 'shape' && obj.shapeType === 'rectangle' && (<div style={{ width: '100%', height: '100%', backgroundColor: `rgba(${obj.fillColor.r}, ${obj.fillColor.g}, ${obj.fillColor.b}, ${obj.fillColor.a})`, border: `${obj.borderWidth}px solid rgb(${obj.borderColor.r}, ${obj.borderColor.g}, ${obj.borderColor.b})`, }} />)}
            </Rnd>
        ))}
      </div>
    </div>
